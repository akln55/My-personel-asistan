...
function save(){ localStorage.setItem('harun_pa_v1', JSON.stringify(state)) }
function load(){ const s=localStorage.getItem('harun_pa_v1'); if(s) state=JSON.parse(s); render(); }
...
document.getElementById('setAlarm').onclick=async ()=>{
  ...
  if(!ok){ alert('Bildirim izni gerekli.'); return; }
  const [hh,mm]=document.getElementById('wakeTime').value.split(':').map(Number);
  scheduleDaily(hh,mm);
  alert('Alarm tarayıcı bildirimi olarak ayarlandı.');
}
...
function scheduleDaily(hh,mm){
  ...
  alarmTimer=setTimeout(()=>{
    notify('Uyanma Zamanı', `Saat ${hh}:${mm} — Kalk!`);
    setInterval(()=>notify('Uyanma Zamanı','Gün başlıyor!'), 24*3600*1000);
  }, until);
}
...
document.getElementById('ask').onclick=()=>{
  ...
  if(q.match(/alışveriş|almalı|almam/)){ const num=(q.match(/\d+/)||[0])[0]; const price=Number(num);
    if(price<=state.balance*0.05) ansEl.innerText='Alınabilir. Bütçeni bozmaz.';
    else if(price<=state.balance*0.15) ansEl.innerText='Düşünebilirsin ama şart değil.';
    else ansEl.innerText='Alma. Birikim hedefini bozar.'; return; }
  if(q.includes('ödev')) { ansEl.innerText='Önce en acilini yap, 25-30 dakika çalış. Pomodoro uygundur.'; return; }
  ...
  ansEl.innerText='Bunu daha sonra ChatGPT ile genişletebilirsin. (Yerel modda cevap veriliyor)';
}
